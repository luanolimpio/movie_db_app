...
void main() {
  const tPersonResultModel = PersonResultModel(
    adult: false,
    gender: 1,
    id: 224513,
    knownFor: [
  ...
  test('Should be a subclass of PersonResultEntity', () {
    expect(tPersonResultModel, isA<PersonResultEntity>());
  });

  test('Should return a valid PersonResultModel when JSON is given', () async {
    final Map<String, dynamic> jsonMap = json.decode(tJson);
    final result = PersonResultModel.fromJson(jsonMap);
    expect(result, isA<PersonResultEntity>());
  });
}